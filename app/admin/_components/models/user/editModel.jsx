...
export default function EditUser({ userInfo, isOpenModel, onClose, refresh }) {
  ...
  useEffect(() => {
    if (userInfo) {
      ...
      setVerified(userInfo.verified || false);
      setStatus(userInfo.status || "win");
      setTotalBalance(userInfo.totalBalance || 0);
    }
  }, [userInfo]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/users/${userInfo._id}`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({ 
          username, 
          email,  // Include password only if it's provided
          verified, 
          status, 
          totalBalance 
        }),
